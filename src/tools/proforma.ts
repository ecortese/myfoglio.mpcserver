...
/** Schema comune per la creazione/aggiornamento di una proforma */
...
export const proformaTools: ToolDefinition[] = [
  {
    name: "proforma_list",
    description: "Recupera la lista delle fatture proforma",
  ...
  },
  {
    ...
    description: "Crea una nuova fattura proforma",
    inputSchema: documentBodySchema,
    handler: async (input) => {
      const { data } = await apiClient.post("/Proforma", input);
      return { content: [{ type: "text" as const, text: JSON.stringify(data, null, 2) }] };
    },
  },
  {
    name: "proforma_update",
    description: "Aggiorna una fattura proforma esistente tramite ID",
    inputSchema: z.object({
      id: z.number().int().positive().describe("ID della fattura proforma da aggiornare"),
      ...documentBodySchema.shape,
    }),
    handler: async (input) => {
      const { id, ...body } = input as { id: number } & Record<string, unknown>;
      const { data } = await apiClient.put("/Proforma", body, { params: { id } });
      return { content: [{ type: "text" as const, text: JSON.stringify(data, null, 2) }] };
    },
  },
  {
    name: "proforma_delete",
    description: "Elimina una fattura proforma tramite ID",
    inputSchema: z.object({
      id: z.number().int().positive().describe("ID della fattura proforma da eliminare"),
    }),
    handler: async (input) => {
      const { id } = input as { id: number };
      const { data } = await apiClient.delete("/Proforma", { params: { id } });
      return { content: [{ type: "text" as const, text: JSON.stringify(data, null, 2) }] };
    },
  },
  {
    name: "proforma_next_code",
    description: "Ottieni il prossimo numero/codice disponibile per le fatture proforma",
    inputSchema: z.object({
      SectionalID: z.number().int().optional().describe("ID del sezionale per cui calcolare il prossimo codice"),
    }),
    handler: async (input) => {
      const { data } = await apiClient.post("/Proforma/action/NextCode", input);
      return { content: [{ type: "text" as const, text: JSON.stringify(data, null, 2) }] };
    },
  },
  {
    name: "proforma_transmit",
    description: "Trasmette una fattura proforma (invio elettronico) tramite ID",
    inputSchema: z.object({
      id: z.number().int().positive().describe("ID della fattura proforma da trasmettere"),
    }),
    handler: async (input) => {
      const { id } = input as { id: number };
      const { data } = await apiClient.post("/Proforma/action/Transmit", {}, { params: { id } });
      return { content: [{ type: "text" as const, text: JSON.stringify(data, null, 2) }] };
    },
  },
  {
    name: "proforma_calculate_totals",
    description: "Calcola i totali di una fattura proforma senza salvarla",
    inputSchema: documentBodySchema,
    handler: async (input) => {
      const { data } = await apiClient.post("/Proforma/action/CalculateTotals", input);
      return { content: [{ type: "text" as const, text: JSON.stringify(data, null, 2) }] };
    },
  },
];
